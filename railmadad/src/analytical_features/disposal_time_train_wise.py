...
def disposal_time_train_wise(request):
    try:
        ...
        if request.method == "POST":
            ...
            for tr in checked:
                disposal_time =  DBQuery.disposal_time_train_wise(start_date,end_date,complain_type,tr)
                ...
                if query_count == 0:
                    ...
                    main_data.append(float('%.2f' % avg_query))

            make_dict = dict(zip(checked, main_data))
        ...
        return render(request, 'railmadad/disposal_time_train_wise.html',context)
    except:
