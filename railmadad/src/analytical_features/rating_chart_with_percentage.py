...
def rating_percentage(request):
    try:
        ...
        if request.method == "POST":
            ...
            if len(complain_type) != 0:
                dataa = DBQuery.rating_chart_with_percentage_data_complain_type(main_trains,complain_type, start_date,end_date)
            else:
                dataa = DBQuery.rating_chart_with_percentage_data_with_no_complain_type(main_trains,start_date,end_date)
        ...
        return render(request, "railmadad/rating_percentage.html", context)
    except:
