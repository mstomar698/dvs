...
def complain_type_line_chart(request, complain, start_date, end_date, clicked_date):
    try:
        print(clicked_date)
        train_numbers_list = []
        complains_list = []
        ...
        if complain == "DISPOSAL_DATE_WISE":
            ...
            problem_types = []
            problem_types = DBQuery.complain_type_interactive_disposal_train(request,complains_list,start_date,end_date)
                ...
                if (start_date == "None" or end_date == "None" or train_numbers_list == [""] or complains_list == [""]):
                    if (
                        ...
                        problem_types = Main_Data_Upload.objects.values_list(
                            ...
                            registration_date__range=[ 
                                dt.strptime(f"{clicked_date} {START_TIME}" , '%Y-%m-%d %H:%M:%S').astimezone(IST),
                                dt.strptime(f"{clicked_date} {END_TIME}" , '%Y-%m-%d %H:%M:%S').astimezone(IST)
                            ]
                        )
        ...
        return render(request, "railmadad/complain_type_line_chart.html", context)
    except:
