...
def old_download_data_csv(request):
    ...
    if request.method == "POST":
        ...
        if delta.days <= -1:
            ...
            writer.writerow(['S.NO', 'Ref. No.','Registration Date','Closing Date','Disposal Time',
                            'Train/Station', 'COACH TYPE', 'COACH NO', 'RAKE NO', 'Escorting Staff',
                            'Type', 'Sub Type', 'Div', 'Owning Zone', 'Owning Div',
                            'Dept', 'Rating', 'Status', 'Complaint Description', 'Remarks',
                            'No. of times forwarded', 'PNR/UTS No', 'Coach Type', 'Coach No.', 'Feedback Remarks',
                            'Upcoming Station', 'Mobile No./Email Id', 'Physical Coach No', 'Train Name', 'Current User Id',
                            'Current User Mobile No',
            ...
            users = Main_Data_Upload.objects.all().values_list(
                            ...
                            'problem_type', 'sub_type', 'div', 'owning_zone', 'owning_div', 
                            'dept', 'rating', 'status', 'complaint_discription', 'remark',
                            'number_of_time_forwarded', 'pnr_utc_number', 'physical_coach_type', 'train_coach_number', 'feedback_remark',
                            'upcoming_station', 'mobile_number_or_email', 'physical_coach_number', 'train_name', 'current_user_id',
                            'mobile_number_or_email',
                            'created_at','updated_at','created_by','updated_by'
                            ).filter(
                                registration_date__range=[dt.strptime(f"{start_date} {START_TIME}", '%Y-%m-%d %H:%M:%S').astimezone(IST),f"{end_date} {END_TIME}+00:00"],
            ...
            for formated_user in formated_users:             
                ...
                list_formated_user[2] = list_formated_user[2].strftime("%d-%m-%y %H:%M")
                list_formated_user[3] = list_formated_user[3].strftime("%d-%m-%y %H:%M")
    ...
    return render(request, 'railmadad/old_download_data.html',context)
